...
export class BarrasEmitidasComponent implements AfterViewInit, OnInit, OnDestroy {
  ...
  private create() {    
    ...
    this.X = d3.map(this.data, (d: any) => d.date);
    this.Y = d3.map(this.data, (d: any) => d.value);
  ...
  private setForms() {
    ...
    let rects = this.svg
      ...
      .enter()
      .append('rect')
      .attr('x', (_: any, i: number) => this.xScale(this.X[i]))
      .attr('width', widthSize)
      .attr('height', 0)
      .attr('y', (_: any, i: number) => this.yScale(50))
