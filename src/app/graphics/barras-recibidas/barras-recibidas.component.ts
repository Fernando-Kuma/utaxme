...
@Component({
  selector: 'app-barras-recibidas',
  templateUrl: './barras-recibidas.component.html',
  styleUrls: ['./barras-recibidas.component.css']
})



export class BarrasRecibidasComponent implements AfterViewInit, OnInit, OnDestroy {

  isDarkTheme: Observable<boolean>;
  public dark: boolean = false;

  @ViewChild('barcharrecibida') svgBarChart: ElementRef;
  @Input() sizex: number;
  @Input() sizey: number;
    ...
    .html((d)=> {
      ...
      if(this.viewTooltip){
        if(this.dark){
          ...
          return " <div class='TooltipGenerated' style='width: "+ width +"px; height: "+ height +"px; border-radius: 3px; line-height: "+ lineHeight +"px; text-align: center; padding-top: 5px; font-size: "+ fontSize +"px; font-weight: "+ fontWeight +"; opacity: .8; background:"+ this.color +";'><span style='color: #0B112A !important'>"+formatShort(new Date(d.target.__data__.date))+"</span><br><span style='font-size: "+ (fontSize+1) +"px; font-weight: "+ (fontWeight+100) +";'><b>"+ (this.general ? 'Activos: ': '') + d.target.__data__.value +''+ this.unit +''+(this.tickets ? this.tooltipLabel : '') + "</b></span></div>";
      }
      }
    });
